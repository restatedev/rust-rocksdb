...
use rust_rocksdb::{
    ...
    FifoCompactOptions, IteratorMode, MultiThreaded, Options, PerfContext, PerfMetric, RateLimiter,
...
/// Smoke test: two DBs configured with one `RateLimiter` open, write, and
/// read. The RocksDB C API exposes no rate-limiter accounting, so this cannot
/// observe that the limit is actually shared between the two instances.
#[test]
fn shared_ratelimiter_smoke_test() {
