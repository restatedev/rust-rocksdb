...
impl TablePropertiesCollectorFactory for Factory {
    ...
    fn create(&self, ctx: TablePropertiesCollectorContext) -> Option<Collector> {
        assert!(ctx.num_levels > 0);
        if self.0.decline.load(Ordering::Relaxed) {
            return None;
        }
...
fn collector_flush_filter_errors_and_ownership() {
    ...
    // A declining factory skips the collector entirely: no collector is
    // created and the flushed table carries none of its properties.
    let created_before = state.created.load(Ordering::Relaxed);
    state.decline.store(true, Ordering::Relaxed);
    db.put(b"declined", b"v").unwrap();
    db.flush().unwrap();
    assert_eq!(state.created.load(Ordering::Relaxed), created_before);
    assert_eq!(state.flushed.lock().last(), Some(&None));
    state.decline.store(false, Ordering::Relaxed);
    drop(db);
    assert_eq!(
