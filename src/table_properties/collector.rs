...
pub trait TablePropertiesCollectorFactory: Send + Sync + 'static {
    ...
    /// Creates the collector for one table, or returns `None` to decline.
    /// Declining skips every collector callback for that table and is the
    /// cheap way to opt out of files this factory is not interested in.
    fn create(&self, context: TablePropertiesCollectorContext) -> Option<Self::Collector>;
...
impl<F: TablePropertiesCollectorFactory> TablePropertiesCollectorFactoryCallback<F> {
    unsafe extern "C" fn create_collector(
        ...
        unsafe {
            ...
            // RocksDB accepts a null collector as "do not collect for this table".
            let Some(collector) = factory.create(context) else {
